...
const app = (difficulty) => {
  ...
  const game = () => {
    ...
    if (tetromino) {
      ...
      if (!isValid(tetromino.matrix, tetromino.col, tetromino.row, playArea)) {
        tetromino.row--;
        placeTetromino();
      }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.which === 40) {
      rapidFallDown(tetromino, playArea, placeTetromino);
    }
    ...
    if (e.which === 39) {
      moveOnClickRigth(tetromino, playArea);
      console.log("right");
    }
  });
  ...
  pauseBtn.addEventListener("click", () => {
    cancelAnimationFrame(requestAnimationId);
    showGameMessage(context, canvas, "PASUE");
    pauseBtn.disabled = true;
